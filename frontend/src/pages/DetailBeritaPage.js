...
const DetailBeritaPage = () => {
  ...
  const fetchData = async () => {
    try {
      const beritaRes = await fetch(`${process.env.REACT_APP_RAILWAY_URL}/api/berita/${id}`);
      const beritaData = await beritaRes.json();
      setBerita(beritaData);

      const komentarRes = await fetch(`${process.env.REACT_APP_RAILWAY_URL}/api/komentar/berita/${id}`);
      const komentarData = await komentarRes.json();
      setKomentar(komentarData);

      if (token) {
        const userRes = await axios.get(`${process.env.REACT_APP_RAILWAY_URL}/api/auth/me`, {
  ...
  const handleKomentarSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_RAILWAY_URL}/api/komentar`,
  ...
  const handleHapusKomentar = async (komentarId) => {
    ...
    try {
      await axios.delete(`${process.env.REACT_APP_RAILWAY_URL}/api/komentar/${komentarId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
